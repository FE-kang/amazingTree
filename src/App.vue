...
<template>
  <div style="padding: 12px;">
    <div class="form-item">
      <input type="text" v-model="filterName" style="flex: 1;" @input="filterNode" /><button
        @click="filterNode">筛选节点</button>
    </div>
    <div class="tree-wrapper" :style="{ width: boxW + 'px', height: boxH + 'px' }">
      <!-- @vue-generic {import('@/types').NodeItem} -->
      <AmazingTree :data="data" :props="{ value: 'uuid', label: 'name', children: 'children' }"
        :current-node-key="initNodeKey" :allow-drop="allowDrop"
